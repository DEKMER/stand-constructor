...
import { Camera, ZoomIn, ZoomOut, Mail, Clock, Award } from 'lucide-react';
...
export const HeadPersonBlock: React.FC<HeadPersonBlockProps> = ({
  ...
  return (
    <div className="relative flex flex-col h-full min-h-0 bg-white/95 backdrop-blur-sm rounded-xl shadow-xl border-2 border-rose-300/80 overflow-hidden text-left p-2.5 justify-between">
      ...
      {/* Main Large Portrait Frame (Vertically Adaptive) */}
      <div className="relative group w-full flex-1 min-h-[120px] rounded-lg overflow-hidden bg-slate-100 border border-rose-200 shadow-inner flex items-center justify-center mb-1.5 select-none">
        {headPerson.photoUrl ? (
          <img
      ...
      <div className="flex-shrink-0 flex flex-col space-y-0.5">
          ...
          {isEditable ? (
            <input
              ...
              placeholder="ФАМИЛИЯ"
              className="font-surname font-black text-2xl xl:text-3xl text-[#7a0c22] tracking-wider border-b border-transparent hover:border-rose-300 focus:border-rose-600 focus:outline-none transition-colors w-full leading-tight truncate"
            />
          ) : (
          ...
          {isEditable ? (
            <textarea
              ...
              rows={2}
              className="text-[11px] font-bold uppercase tracking-wider text-[#9e1432] leading-tight border border-transparent hover:border-rose-300 focus:border-rose-600 focus:outline-none rounded p-0.5 transition-colors resize-none"
            />
          ) : (
        ...
        <div className="flex items-center gap-1.5">
          ...
          {isEditable ? (
            <input
              ...
              className="w-full border-b border-transparent hover:border-slate-300 focus:border-rose-600 focus:outline-none text-[10.5px]"
            />
          ) : (
